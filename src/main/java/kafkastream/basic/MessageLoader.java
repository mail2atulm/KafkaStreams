...
public class MessageLoader {
  ...
  private void createTopic(String topicName) {
    ...
    try {
      ...
      CreateTopicsResult rs = admin.createTopics(Collections.singleton(topic));
  ...
  private void sendMessage(int i) throws Exception {
    ...
    String msgKey = "k1"; //Default Key is K1
    String msgVal = "test message"+(i);

    if (i % 2 == 0) {
      msgKey = "K2";
      msgVal = "example_"+msgVal;

    }
    ProducerRecord<String, String> record = new ProducerRecord<>(Util.sourceTopic, msgKey, msgVal);
